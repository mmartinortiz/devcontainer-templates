...
## CI linting & dependency updates

- `.github/workflows/ci-templates.yaml`'s `lint-actions` job lints every `.github/workflows/*.yaml` with `actionlint` (a pinned release binary) and runs `shellcheck` over `.github/actions/*/*.sh` and `.github/scripts/validate-template.sh`. It has no `needs` (runs in parallel with `detect`) but is included in `ci-templates-ok`'s `needs` list, so it's part of the single required check described above.
- Every external `uses:` ref is pinned to a full-length commit SHA with a trailing `# vX.Y.Z` comment, e.g. `actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1 # v7.0.1`. This is enforced by the repo setting Settings → Actions → General → "Require actions to be pinned to a full-length commit SHA" — workflow runs are blocked if a `uses:` floats on a tag or branch instead. Local composite-action refs (`uses: ./.github/actions/*`) are exempt and intentionally left unpinned.
- `.github/dependabot.yml` owns bumping those SHAs/comments going forward: `github-actions` ecosystem only, checked monthly, all updates grouped into a single PR. The `docker` ecosystem is deliberately excluded — the one versionable image ref under `images/`, `ghcr.io/astral-sh/uv` via `ARG UV_IMAGE_TAG` in `images/base/Dockerfile`, is already bumped weekly by `update-base-image-versions.yaml` (which also updates the `images/base/README.md` table and runs a pre-PR build + smoke test, see below); the other `FROM`, `mcr.microsoft.com/devcontainers/base:ubuntu`, is an intentionally floating tag that Dependabot has nothing to pin.
...
- `images/base/Dockerfile` — Ubuntu base (`mcr.microsoft.com/devcontainers/base:ubuntu`) + fish/vim (apt), uv/uvx (copied from `ghcr.io/astral-sh/uv`), Starship (pinned GitHub release binary), `prek` (via `uv tool install`, relocated `UV_TOOL_DIR=/opt/uv-tools` so the non-root `vscode` user can execute it — `uv tool install` defaults to `~root/.local/share/uv/tools`, unreadable by other users), `opencode` (pinned GitHub release binary, glibc build — no fish completions since `opencode completion` is a yargs bash-only generator that ignores its shell argument), and `delta` (pinned GitHub release binary, wired in as git's system-wide pager/diff filter). Also sets an `org.opencontainers.image.description` LABEL listing the exact versions of all five tools, generated from the same `ARG`s at build time (visible via `docker inspect`, no need to pull and run each `--version`). Pre-creates `~/.local`, `~/.local/bin`, `~/.local/share`, `~/.local/state`, `~/.cache`, `~/.config` owned by `vscode` before `USER vscode` — Docker creates missing bind/volume mount-point parent directories as `root:root`, so a consumer devcontainer mounting a volume under e.g. `~/.local/share/opencode` would otherwise leave `~/.local` root-owned, breaking tools (like opencode itself) that need to `mkdir` under `~/.local/state`.
...
- Versions are pinned via build ARGs (`UV_IMAGE_TAG`, `STARSHIP_VERSION`, `PREK_VERSION`, `OPENCODE_VERSION`, `DELTA_VERSION`) with defaults baked in — bump these instead of relying on floating `latest` tags.
