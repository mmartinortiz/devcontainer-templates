...
- `.github/workflows/build-images.yaml` has two jobs: `test` (builds single-arch with `load: true`, runs `test/images/base/test.sh`) must pass before `build` (multi-arch buildx, push to GHCR only on merge to `main`, build-only on PRs).
